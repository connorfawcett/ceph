...
ceph::io_exerciser::Seq11::Seq11(std::pair<int,int> obj_size_range, int seed) :
  IoSequence(obj_size_range, seed), count(0), doneread(false), donebarrier(false)
{
...
std::unique_ptr<ceph::io_exerciser::IoOp> ceph::io_exerciser::Seq12::_next()
{
  ...
  count++;
  barrier = true;
  return SingleWriteOp::generate((count * obj_size) - overlap, obj_size + overlap);
}
...
ceph::io_exerciser::Seq14::Seq14(std::pair<int,int> obj_size_range, int seed) :
  IoSequence(std::make_pair(0, obj_size_range.second), seed), offset(0), step(1)
{
